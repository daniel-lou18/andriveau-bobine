...
Relationship to **`@andriveau-bobine/suggest`**: suggest supplies `ResolvedRue` / `rueId` (handoff types in the suggest package); lookup consumes opaque `rue_id` only. Shared packages do not depend on each other; the web app composes both hooks in `App.tsx`.
