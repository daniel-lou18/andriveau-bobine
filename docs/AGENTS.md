...
- **Wrangler binding:** `wrangler.jsonc` declares `d1_databases` with binding **`DB`** and `migrations_dir` **`drizzle`** (SQL migrations generated by Drizzle Kit live there). After changing bindings, run **`npm run types`** in `apps/api` (or **`npm run types:api`** from the root).
...
- **Migrations:** Edit or add modules under `src/db/schemas/` and wire them in **`src/db/schema.ts`**, then:
  - `npm run db:generate` — generate SQL under `apps/api/drizzle/`
  - `npm run db:migrate:local` — apply to **local** D1
  - `npm run db:migrate:remote` — apply to **remote** D1 (after `database_id` is correct)
- **Commit** generated files under `apps/api/drizzle/` with the rest of the repo.
