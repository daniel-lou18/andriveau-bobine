...
import { parseExtractionBatch, type ExtractionBatch } from "./validate";

export type LoadBatchResult =
  | { status: 200; body: LoaderReport }
...
export async function loadBatch(
  ...
  const parsed = parseExtractionBatch(payload);
  if (!parsed.ok) {
    return {
      status: 400,
      body: { error: "Invalid extraction batch JSON", issues: parsed.issues },
    };
  }

  const batch: ExtractionBatch = parsed.data;
  ...
  for (const rec of batch.logical_records) {
    ...
    // Interchange may use accents (e.g. "cité"); `voie_types.code` is ASCII (e.g. "cite").
