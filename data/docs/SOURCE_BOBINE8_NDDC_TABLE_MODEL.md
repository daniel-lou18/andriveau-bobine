...
| `bis`, `ter`, …, `septies`                        | French **suffix** forms (e.g. `4 bis`, `4ter`, `59 -> 61 bis`) — suffix stays on the **numeral token** it modifies; modeled ranks through `septies` per **`docs/EXTRACTION.md`** / `apps/api/src/lib/suffix.ts`                                                                                  |
